...
- **No rating change.** No score-aware likelihood, no shadow model, no alternative rating runtime, no change to Glicko, initial ratings, or player-visible rating output.
...
- **Types** (`src/types.ts`): `Session` gains `defaultScoringFormat`; `Match` gains `scoringFormat`. Both are required on new records and reconstructed on load.
