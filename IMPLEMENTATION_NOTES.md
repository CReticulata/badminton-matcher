...
- `generateRound`：自願休息者先剔除 → 其餘洗牌後依當日上場次數穩定排序（並列隨機）→ 取前 4（單打取 2）上場，其餘＋自願者休息 → `balanceTeams` 在 3 種拆隊法中取兩隊 rating 總和差最小者。公平嚴格優先於強度平衡。
- 測試涵蓋：4/5/8 人、遲到者連打不休息、自願休息、人數不足回 null、並列隨機性、公平優先於平衡、10 回合模擬的公平不變量（任兩人上場次數差 ≤1）。
...
4. **下一場預告與實際產生各自隨機**：並列隨機導致預告與按下「產生」的結果可能不同，design.md 已標明「僅預告」。
5. **手動覆寫的時序**：覆寫存事件、重播於比賽之間，若使用者「先覆寫再修改更早的歷史」會照真實時間軸重播（正確語意）。
6. **編輯歷史僅開放改比分**：改對戰組合＝刪除該場重新打一場的語意，用刪除＋重新記錄即可達成，UI 未做組合編輯器。
7. **瀏覽器實測涵蓋**：新增球員、開場次、產生分組、交換（含休息者）、對戰畫面（橫/直）、平手擋下、記分、rating 更新、歷史改分重算、重新整理持久化。未逐一實測：離場/重新加入按鈕、單打畫面、CSV 按鈕點擊（皆有單元測試或同構程式路徑覆蓋）。
